...
{
    double ans;
    ans = sqrt(pow(s[0] - d[0], 2.0) + pow(s[1] - d[1], 2.0) + pow(s[2] - d[2], 2.0));
    return(ans);
}
...
{
    double ans;
    ans = 10 * log10(pow(10, x/10.0)+pow(10, y/10.0));
    return(ans)
}
...
public:
    ...
    void init(int obs_len, int act_len)
    {
        self_obs_len = obs_len
        self_act_len = act_len
    }
...
class BS
{
    ...
    double subframe_duration = 0.01; //s = 10ms
    int transmit_power_list[3];
...
public:
    ...
    void init(Simulator& sim, string BStype_x, int id_x, int cell_color_x, double x_pos, double y_pos, double z_pos, int cell_id_x, char* config_p_x)
    {
        ...
        UE connecting_UE_ul[];
        UE connecting_UE_dl[];
        int max_antenna_ul = 4
        int max_antenna_dl = 4
        int* observation[];
        int* throughput_ul_list;
        int* throughput_dl_list;
        int* sentdata_ul_list;
        int* sentdata_dl_list;
        
        UE c_connecting_UE_ul[];
        UE c_connecting_UE_dl[];
        int* ans_ul[];
        int* ans_ul_num[];
        int* ans_dl[];
        int* ans_dl_num[];
        ...
        if (strcmp(BStype, "SBS") == 0){
            transmit_power_list = {40, 60, 80};
        }else if (strcmp(BStype, "MBS") == 0){
            transmit_power_list = {40, 60, 80};
        }else{
            exit(1);
        }
        
        transmit_power = d2w(transmit_power_list[0])
        ...
        if (strcmp(sim.mode, "conv") == 0){
            config = config_p[0]       
        }else if (strcmp(sim.mode, "schD") == 0){
            config = config_p[1] 
        }else if (strcmp(sim.mode, "DRL") == 0){
            config = config_p[1] 
        }
            
        printf("config: %s", config)

        config_len = strlen(config_p)
        
        printf("%s: %f: %d", get_name(), get_xyz(), cell_id)
        ...
        if (strcmp(get_type(),"SBS") == 0){
            obs_len = int(24 * sbs_interval / 10) #160#12 #10 #200
        }else{
            obs_len = int(24 * mbs_interval / 10) #200
        }
        
        observation[obs_len] = {0}  

        //
            
        if strcmp(sim.mode, "DRL"):
            gen_bs_agent()
    ...
    void set_c_connecting_UE()
    {
        UE c_connecting_UE_ul[];
        UE c_connecting_UE_dl[];
        memcpy(c_connecting_UE_ul, connecting_UE_ul, sizeof(connecting_UE_ul));
        memcpy(c_connecting_UE_dl, connecting_UE_dl, sizeof(connecting_UE_dl));
    }
        
    void set_prev_action(int x)
    {
        int prev_action;
        prev_action = x
    }
    ...
    void set_action_history(int x)
    {
        vector<int> action_history;
        
        if strcmp(get_type(), "MBS"):
            action_history = [x];
        else:
            action_history.Push_back(x);
    }
    ...
    void find_agent_next_bs()
    {

        BS agent_next_bs;
        
        if strcmp(get_type(), "MBS"){
            vector<BS> same_cell_bs;
            int small_cell_color = 10000;
            for (int bs = 0; bs < sim.BS_list.size(); bs++){
                if strcmp(get_type(), "MBS") and (sim.BS_list[bs].get_cell_color() == cell_color+1):
                    same_cell_bs.Push_back(bs)
                if strcmp(get_type(), "MBS") and (sim.BS_list[bs].get_cell_color() < small_cell_color):
                    small_cell_color = sim.BS_list[bs].get_cell_color()
            }
