...
from . import darwinian_rbm_meta_rapper as dw
...
#        )
    ...
    def sim_set_state_darwinian(self, config, x):
        ##print("config:"+str(config)+"id:"+str(self.id*10+x))
...
#######################
    def execute_sim_darwinian(self, position_list):
        ...
        w1 = np.empty((0, self.NN_num), dtype=np.float32)
        for deme_id, deme_inside in enumerate(deme_list):
            #print("deme_inside shape:"+str(deme_inside)+",num:"+str(deme_id))
            w2 = np.empty(0, dtype=np.float32)
            for o_env, output in enumerate(deme_inside):
                #print("output:"+str(output)+",o_env:"+str(o_env))
                ...
                w2 = np.append(w2, np.float32(reward))
            self.Deme.py_return_fitness(w2, deme_id)
            w1 = np.append(w1, [w2])
        #print("w1:"+str(w))
        ...
        w1 = np.empty((0,2), dtype=np.float32)
        for deme_id, deme_inside in enumerate(deme_list):
            w2 = np.empty(0, dtype=np.float32)
            for o_env, output in enumerate([deme_inside[0], deme_inside[1]]):
...
#            for o_env, output in enumerate(deme_inside):
                ...
                w2 = np.append(w2, np.float32(reward))
            self.Deme.py_return_mutate_fitness(w2, deme_id)
            w1 = np.append(w1, [w2])
        #print("w2:"+str(w))
...
#######################
    ...
    def execute_darwinian(self, position_list):
        ...
        for bs in self.BS_list:
            ...
            for deme_id, deme_inside in enumerate(deme_list):
                ...
                for o_env, output in enumerate(deme_inside):
                    ...
                    for j in range(10):
                        ...
                        #print("output2:"+str(output))
                        bs.set_state_darwinian(output, self.time_slot)                    
                        #for i in self.BS_list:
                        #    i.set_state(self.time_slot)
                    ...
                    w2 = np.append(w2, np.float32(reward))
                bs.Deme.py_return_fitness(w2, deme_id)
                w1 = np.append(w1, [w2])
            #print("w1:"+str(w))
            ...
            for deme_id, deme_inside in enumerate(deme_list):
                ...
                for o_env, output in enumerate([deme_inside[0], deme_inside[1]]):
                    ...
                    w2 = np.append(w2, np.float32(reward))
                bs.Deme.py_return_mutate_fitness(w2, deme_id)
                w1 = np.append(w1, [w2])
            #print("w2:"+str(w))
        ...
        for j in range(10):
            ...
            for i in self.BS_list:
                i.set_state_darwinian(i.best_output, self.time_slot)                    
            #for i in self.BS_list:
            #    i.set_state(self.time_slot)
