...
#from . import darwinian_rapper as dw
from . import darwinian_rbm_rapper as dw
...
def dsum(x,y):
    return(10 * math.log10(math.pow(10, x/10.0) + math.pow(10, y/10.0)))
...
class BS():
    ...
    def __init__(self, sim, BStype, id_x, cell_color, x_pos, y_pos, z_pos, cell_id, config_p):
        ...
        color_chr = ["yo","mo","co","ro","go","bo","ko","ks"]
        print("plot("+str(self.get_xyz()[0])+", "+str(self.get_xyz()[1])+", '"+str(color_chr[self.cell_color])+"')")
    ...
    def get_bs_sentdata_total(self):
    
        print("sentdata_ul:"+str(self.sentdata_ul_list))
        print("sentdata_dl:"+str(self.sentdata_dl_list))
    
        ans1 = sum(self.sentdata_ul_list)
        ans2 = sum(self.sentdata_dl_list)
...
#        )
    ...
    def darwinian_init_set_action(self, t):
    
        self.Deme = dw.PyDeme()
        self.Deme.py_evolution_initialization(t)
        
        self.Deme.py_evolution_gen_environment(t)
        deme_list = self.Deme.py_evolution_gen_update(t)	
        #py_evolution_gen_calcfitness(t)
        return(deme_list)


       
    def darwinian_evol_set_action(self, t):
    ...
    def darwinian_learning(self, t):

        #py_evolution_gen_calcfitness(t)
        best_output = self.Deme.py_evolution_gen_calcbestfitness(t)
        self.best_output = best_output
        
    def darwinian_mutation(self, t):    
        
        deme_list = self.Deme.py_evolution_gen_selection(t)
        #py_evolution_gen_calcmutation(t)
        return(deme_list)
        
    def darwinian_mutation_replacement(self, t):
        self.Deme.py_evolution_gen_replacemutation(t)

    def darwinian_mutation_learnbest(self, t):
        self.Deme.py_evolution_gen_learnbest(t)

    def darwinian_mutation_learnworst(self, t):
        self.Deme.py_evolution_gen_learnworst(t)
    ...
    def set_state_darwinian(self, config, x):
    ...
    def sim_set_state_darwinian(self, config, x):
        #print("config:"+str(config))
        if config[self.id * 10 + x] == 1.0:
            self.state = 'U'
        elif config[self.id * 10 + x] == -1.0:
            self.state = 'D'
        else:
            self.state = 'A'

    def sim_set_state_darwinian_quad(self, config, x):
        if (config[self.id * 2] == -1.0) and (config[self.id * 2+1] == -1.0):
            self.config = self.config_p[1]
        elif (config[self.id * 2] == -1.0) and (config[self.id * 2+1] == 1.0):
            self.config = self.config_p[3]
        elif (config[self.id * 2] == 1.0) and (config[self.id * 2+1] == -1.0):
            self.config = self.config_p[5]
        else:
            self.config = self.config_p[7]

        self.state = self.config[x] 

    def sim_set_state_darwinian_random(self, x):
...
#        self.c_connecting_UE_dl = self.connecting_UE_dl
    ...
    def gen_random_data(self, time_step):
        ...
        data_size = 0.5 * 32#8 #Mbyte 0.5MB * pow(10, 6) 
        repeat_interval = 100
        ...
        for i_env, i in enumerate(self.inside_UE):
            ...
            for _ in range(self.sim.ul_random[i.get_id()][time_step%repeat_interval]):
                #print("gen_ul")
                i.gen_data(self, data_size, "ul")
                #i.gen_ul_data(self, data_size)        
            for _ in range(self.sim.dl_random[i.get_id()][time_step%repeat_interval]):
                #print("gen_dl")
...
class UE():
    ...
    def __init__(self, sim, UEtype, id_x, cell_color, x_pos, y_pos, z_pos, bs):
        ...
        color_chr = ["yx","mx","cx","rx","gx","bx","kx","k*"]
        print("plot("+str(self.get_xyz()[0])+", "+str(self.get_xyz()[1])+", "+str(self.get_xyz()[2])+", '"+str(color_chr[self.cell_color])+"')")
    ...
    def not_at_gate(self):
        #4270.1503, -40728.6466
        x, y, z = self.get_xyz()
        if math.isclose(x, 4270.1503) and math.isclose(-40728.6466):
            return(False)
        else:
            return(True)
    ...
    def gen_random_data_ue(self, time_step):
        ...
        if self.not_at_gate():
            ...
            for _ in range(self.sim.ul_random[self.get_id()][time_step%repeat_interval]):
                self.gen_data(self.inside_BS, self.data_size, "ul")
                print("gen_random_data_ue_ul:"+str(self.get_name())+", "+str(self.inside_BS.get_name()))
            for _ in range(self.sim.dl_random[self.get_id()][time_step%repeat_interval]):
                self.gen_data(self.inside_BS, self.data_size, "dl")
                print("gen_random_data_ue_dl:"+str(self.get_name())+", "+str(self.inside_BS.get_name()))

    def gen_static_data_ue_var(self, time_step):
        
        #self.gen_data(self.inside_BS, self.data_size, "ul")
        
        if (time_step//100) %2 == 0:
            #self.gen_data(self.inside_BS, 4, "ul")
            self.gen_data(self.inside_BS, 40, "ul")
            print("gen_static_data_ue_ul: 40,"+str(self.get_name())+", "+str(self.inside_BS.get_name())+", "+str(time_step))
        else:
            #self.gen_data(self.inside_BS, 4, "dl")
            self.gen_data(self.inside_BS, 40, "dl")
            print("gen_static_data_ue_dl: -40,"+str(self.get_name())+", "+str(self.inside_BS.get_name())+", "+str(time_step))
        
        #if self.id % 2 == 0:
        #    self.gen_data(self.inside_BS, self.data_size, "ul")
        #    print("gen_static_data_ue_ul:"+str(self.get_name())+", "+str(self.inside_BS.get_name()))
        #else:
        #    self.gen_data(self.inside_BS, self.data_size, "dl")
        #    print("gen_static_data_ue_dl:"+str(self.get_name())+", "+str(self.inside_BS.get_name()))

    def gen_static_data_ue_const(self, time_step):
    ...
    def get_TX_power(self):
        ...
        #print("bs:"+str(bs))
        if bs.get_type() ==  "MBS":
            tmp_power = 10 * math.log10(M) + self.power_control_MBS_UE + self.power_control_alpha * self.sim.path_loss(self, bs)
            ...
            tmp_power = 10 * math.log10(M) + self.power_control_SBS_UE + self.power_control_alpha * self.sim.path_loss(self, bs)
    ...
    def cp_data_list(self):
        ...
        for i in self.data_list:
            ...
            x4 = i[4]
            x5 = i[5]
            self.data_list_copy.append([x0, x1, x2, x3, x4, x5])
                    ...
                    if bs.get_state() == 'U':
                        ...
                        if capacity > 0.0:
                            ...
                            if self.data_list[i_env][1] > capacity:
                                ...
                                up_link.consump_capacity(self.data_list[i_env][1]) ########
                            ...
                            elif  self.data_list[i_env][4] > 50:
                                self.data_list.pop(i_env)#
                                self.delete_link_list_ul.append(up_link)


    def send_test(self, slot_id):

        print("send_test")
        for i_env, (bs, data_size, start_bool, _, _, direction) in enumerate(self.data_list_copy):
            if direction == "ul":
                up_link = self.find_link(self, bs)
                if up_link != None:
                    if bs.get_state() == 'U':
                        capacity = up_link.get_capacity()
                        if capacity > 0.0:
                            self.data_list_copy[i_env][3] = 1
                            if self.data_list_copy[i_env][1] > capacity:
                                self.append_sentdata(self.data_list_copy[i_env], capacity, "ul")
                                up_link.consump_capacity(capacity) ########
                                self.data_list_copy[i_env][1] = self.data_list_copy[i_env][1] - capacity
                            else:
                                self.append_sentdata(self.data_list_copy[i_env], self.data_list_copy[i_env][1], "ul")
                                up_link.consump_capacity(self.data_list_copy[i_env][1]) ########
                                self.data_list_copy[i_env][1] = 0#
                            if self.data_list_copy[i_env][1] == 0:
                                self.data_list_copy.pop(i_env)#
                            elif  self.data_list_copy[i_env][4] > 50:
                                self.data_list_copy.pop(i_env)#
                    ...
                    if bs.get_state() == 'D':
                        ...
                        if capacity > 0.0:
                            ...
                            if self.data_list[i_env][1] > capacity:
                                ...
                                dl_link.consump_capacity(capacity) ########
                                ...
                                dl_link.consump_capacity(self.data_list[i_env][1]) ########
                ...
                if dl_link != None:
                    if bs.get_state() == 'D':
                        ...
                        if capacity > 0.0:
                            self.data_list_copy[i_env][3] = 1
                            if self.data_list_copy[i_env][1] > capacity:
                                self.append_sentdata(self.data_list_copy[i_env], capacity, "dl")
                                dl_link.consump_capacity(capacity) ########
                                self.data_list_copy[i_env][1] = self.data_list_copy[i_env][1] - capacity
                            else:
                                self.append_sentdata(self.data_list_copy[i_env], self.data_list_copy[i_env][1], "dl")
                                dl_link.consump_capacity(self.data_list_copy[i_env][1]) ########
                                self.data_list_copy[i_env][1] = 0#
                            if self.data_list_copy[i_env][1] == 0:
                                #print("data list:"+str(self.data_list[i_env]))
                                self.data_list_copy.pop(i_env)#
...
class Simulator():
    ...
    def __init__(self, mode):
        ...
        #self.ul_random = np.random.poisson(self.arrival_rate/2, (1000, 18000))
        self.ul_random = np.random.poisson(self.arrival_rate, (1000, 18000)) ##max_num self.arrival_rate * self.subframe_duration
        self.dl_random = np.random.poisson(self.arrival_rate, (1000, 18000)) ##max_num
        ###self.ul_random = np.ones((1000, 18000), dtype="int8") * 1 ##max_num self.arrival_rate * self.subframe_duration
        ###self.dl_random = np.ones((1000, 18000), dtype="int8") * 1  ##max_num
...
#            env = env + 1        
    ...
    def sim_darwinian_init_set_action(self, t):
    
        self.Deme = dw.PyDeme()
        self.Deme.py_evolution_initialization(t)
        
        self.Deme.py_evolution_gen_environment(t)
        self.Deme.py_test_versatility(t)
        deme_list = self.Deme.py_evolution_gen_update(t)	
        #py_evolution_gen_calcfitness(t)
        return(deme_list)
    ...
    def sim_darwinian_evol_set_action(self, t):
    ...
    def sim_darwinian_learning(self, t):

        #py_evolution_gen_calcfitness(t)
        best_output = self.Deme.py_evolution_gen_calcbestfitness(t)
        self.best_output = best_output
        
    def sim_darwinian_mutation(self, t):    
        
        deme_list = self.Deme.py_evolution_gen_selection(t)
        #py_evolution_gen_calcmutation(t)
        return(deme_list)
        
    def sim_darwinian_mutation_replacement(self, t):
        self.Deme.py_evolution_gen_replacemutation(t)

    def sim_darwinian_mutation_learnbest(self, t):
        self.Deme.py_evolution_gen_learnbest(t)

    def sim_darwinian_mutation_learnworst(self, t):
        self.Deme.py_evolution_gen_learnworst(t)


    def move_file(self, position_list):
        print("move_step:"+str(self.time_step//10))
        self.position = position_list[self.time_step // 10]
        for i_env, i in enumerate(self.position):
            if (i_env % 2 == 0) and (i_env/2 < len(self.UE_list)):
                ##print("i_env:"+str(i_env))
                self.UE_list[i_env // 2].set_xaxis(self.position[i_env])
                self.UE_list[i_env // 2].set_yaxis(self.position[i_env+1])
    ...
    def path_loss(self, s, r): #3GPP, LOS, d in km
        ...
        if (s_t == "MBS" and r_t == "SBS") or (s_t == "SBS" and r_t == "MBS"):
            ans = 100.7 + 23.5 * math.log10(d)
            #ans = 125.2+36.3 * math.log10(d) #NLOS
        elif (s_t == "SBS" and r_t == "SBS"):
            if d < 2/3:
                ans = 98.4+20 * math.log10(d)
            else:
                ans = 101.9+40 * math.log10(d)
            #ans = 169.36 + 40 * math.log10(d) #NLOS
        elif (s_t == "MBS" and r_t == "MBS"):
            ans = 98.45+20 * math.log10(d) 
        elif (s_t == "MBS" and r_t == "UE") or (s_t == "UE" and r_t == "MBS"):
            ans = 103.4+24.2 * math.log10(d)
            #print("MBS-UE:"+str(ans)+","+str(d))
            #ans = 131.1+42.8 * math.log10(d) #NLOS
        elif (s_t == "SBS" and r_t == "UE") or (s_t == "UE" and r_t == "SBS"):
            #print("SBS-UE")
            ans = 103.8 + 20.9 * math.log10(d)
            #ans = 145.4+37.5 * math.log10(d) #NLOS
        elif (s_t == "UE" and r_t == "UE"):
            if d <= 0.05:
                ans = 98.45+20 * math.log10(d)
            else:
                ans = 40 * math.log10(d) + 175.78 
    ...
    def signal(self, s, d, string):
        ...
        ans6 = s.get_antenna_gain()
        ans5 = d.get_noise_figure()
        ans7 = s.get_noise_figure()
...
#        sinr_bottom = interf + d2w(reciever.noise(r1))
        ...
        print("Link_list vector:"+str(len(self.Link_list)))
...
#######################
    ...
    def execute_sim_random_mobility(self, position_list):
        ...
        for j in range(10):
            ...
            for i in self.BS_list:
                i.sim_set_state_darwinian_random(self.time_slot)                    
            #for i in self.BS_list:
        ...
        for bs in self.BS_list:
            ...
            reward = reward + ans1_1
            reward = reward + ans1_2
        print("finalreward:\t"+str(reward))


        #move xy
        #print("len UE_list:"+str(len(self.UE_list)))
        self.move_file(position_list)  #stop move
        for i in self.UE_list:
...
#            self.find_close_bs(i)
        ...
        # gen new data    
        for i in self.UE_list:
            i.gen_random_data_ue(self.time_step // 10)  #gen random data
            #i.gen_static_data_ue_var(self.time_step // 10)   #gen static data
            #i.gen_static_data_ue_const(self.time_step // 10)   #gen static data
        ...
        #for bs in self.BS_list:
        #    ans1_1, ans1_2 = bs.get_bs_sentdata_total() #reset sentdata
...
#######################
    def execute_sim_darwinian(self, position_list):
#    def execute_sim_darwinian(self):
        ...
        if self.time_step == 0:
            deme_list = self.sim_darwinian_init_set_action(self.time_step)
        else:
            deme_list = self.sim_darwinian_evol_set_action(self.time_step)
        ...
        for o_env, output in enumerate(deme_list):
            ...
            for j in range(10):
                ...
                for bs in self.BS_list:
                    ...
                    bs.sim_set_state_darwinian_quad(output, self.time_slot)                    
                    #for i in self.BS_list:
                    #    i.set_state(self.time_slot)
                ...
                for i in self.UE_list:
                    i.recieve_test(self.time_slot)
                #print(str(self.time_slot) + ": recieve")
            
                for i in self.UE_list:
                    i.send_test(self.time_slot)
                #print(str(self.time_slot) + ": send")
            ...
            w = np.append(w, np.float32(reward))
        print("w1:"+str(w))
        self.Deme.py_return_fitness(w, 0)
            
                    
        self.sim_darwinian_learning(self.time_slot)############!!!!!!!
        print("best_output:"+str(self.best_output))

        deme_list = self.sim_darwinian_mutation(self.time_slot)  
        ...
        for o_env, output in enumerate([deme_list[0], deme_list[1]]):
            ...
            w = np.append(w, np.float32(reward))
        print("w2:"+str(w))
        self.Deme.py_return_mutate_fitness(w, 0)

        deme_list = self.sim_darwinian_mutation_replacement(self.time_slot)  
        #deme_list = self.sim_darwinian_mutation_learnbest(self.time_slot)  
        #deme_list = self.sim_darwinian_mutation_learnworst(self.time_slot)  
...
#            i.cp_data_list()
        ...
        for j in range(10):
            ...
            for i in self.BS_list:
                i.sim_set_state_darwinian_quad(self.best_output, self.time_slot)                    
            #for i in self.BS_list:
            #    i.set_state(self.time_slot)
...
#######################
#######################
    def execute_sim_darwinian_greedy(self, position_list):
...
##################
    def execute_darwinian(self, position_list):
        ...
        for bs in self.BS_list:
            ...
            if self.time_step == 0:
                deme_list = bs.darwinian_init_set_action(self.time_step)
            else:
                deme_list = bs.darwinian_evol_set_action(self.time_step)
                
                #if (i.get_type() == "SBS") and (self.time_step % i.sbs_interval == 0) and (i.skip_flag == 0):
                #    #i.initialize_observation()
                #    i.set_action_darwinian()############!!!!!!!
                #    print("set_action")
            ...
            for o_env, output in enumerate(deme_list):
                ...
                for j in range(10):
                    ...
                    #print("output2:"+str(output))
                    bs.set_state_darwinian_quad(output, self.time_slot)                    
                    #for i in self.BS_list:
                    #    i.set_state(self.time_slot)
                ...
                w = np.append(w, np.float32(reward))
            print("w1:"+str(w))
            bs.Deme.py_return_fitness(w, 0)
            
                    
            bs.darwinian_learning(self.time_slot)############!!!!!!!
            print("best_output:"+str(bs.get_name())+""+str(bs.best_output))

            deme_list = bs.darwinian_mutation(self.time_slot)  
            ...
            for o_env, output in enumerate([deme_list[0], deme_list[1]]):
                ...
                w = np.append(w, np.float32(reward))
            print("w2:"+str(w))
            bs.Deme.py_return_mutate_fitness(w, 0)

            deme_list = bs.darwinian_mutation_replacement(self.time_slot)  
            #deme_list = bs.darwinian_mutation_learnbest(self.time_slot)  
            #deme_list = bs.darwinian_mutation_learnworst(self.time_slot)  
        ...
        for j in range(10):
            ...
            for i in self.BS_list:
                i.set_state_darwinian_quad(i.best_output, self.time_slot)                    
            #for i in self.BS_list:
            #    i.set_state(self.time_slot)
...
#            self.find_close_bs(i)
        ...
        for i in self.UE_list:
            #i.gen_random_data_ue(self.time_step // 10)  #gen random data
            i.gen_static_data_ue(self.time_step // 10)   #gen static data
...
#######################
    def execute_DRL_mobility(self, position_list):
...
#######################
    ...
    def execute_conv_mobility(self, position_list):
...
#            i.move()
        ...
        for i in self.BS_list:
            ...
            i.skip_flag = 0
            if (i.get_type() == "SBS"):
                if (self.time_step % i.sbs_interval == 0):
                    ans1_1, ans1_2 = i.get_bs_sentdata_total()
                    reward = ans1_1 + ans1_2
                else:
                    i.skip_flag = 1
            else:
            ...
            print("skip_flag:"+str(i.skip_flag))
        print("reward_total:"+str(reward_total))
...
#######################
    ...
    def execute_random_mobility(self, position_list):
