...
DEF N = 16#38#20#10  #// number of neurons (200) // if Nk landscape is used, the maximal value of N is 63!
...
DEF NN = 10#100      #// population size of a deme (number of networks)
...
DEF n_v = 16#38#20#10 #N
DEF n_h = 16#38#20#10 #N
DEF epsilon_rbm = 0.1
DEF epoch_rbm = 400
DEF T_rbm = 5
...
cdef extern from "darwinian_rbm_rap.h":
    ...
    cdef struct DEME:
        float w[NN], minW, maxW, avgW, input_t[NN][N], output[NN][N], test[NN][N], maxOutput[N], weight[NN][N][N], wworstW;
...
cdef class PyDeme:
    ...
    def py_evolution_gen_learnbest(self, t):
        self.cDeme.evolution_gen_learnbest(t)
        
        #test
        #i=0
    
    def py_evolution_gen_learnworst(self, t):
        self.cDeme.evolution_gen_learnworst(t)
        
        #test
        #i=0
    
    cpdef py_return_fitness(self, np.ndarray[np.float32_t, ndim=1] w, d):
        #cdef np.ndarray w_a = np.empty(d, dtype=np.float32)
        #w_a = np.append(w_a, w)
        cdef np.ndarray[np.float32_t, ndim=1] w_a
        w_a = w
        
        #cdef float *w_c = <float *>w_a.data
        cdef float *w_c = <float *>malloc(d * sizeof(float))
        w_c = <float *>w_a.data
