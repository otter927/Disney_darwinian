from hetnet_hopf_original_meta import HetNet
...
#position_file1 = "data/position09.csv"
#position_file2 = None
#position_file3 = "data/position03.csv"

#position_file1 = "data/position0.1_0.1.csv"
#position_file1 = "data/position0.9_1.0.csv"
position_file1 = args[1]#"data/position0.9_0.1.csv"
#position_file1 = "data/position0.5_0.5.csv"
position_file2 = None
position_file3 = "data/position0.5_0.5.csv"


if position_file1 is not None:
   f = open(position_file1)
   reader = csv.reader(f)
   l = [row for row in reader]
   position_list1 = [[int(v) for v in row] for row in l]
   #print("position_list1:"+str(position_list1)) 

if position_file2 is not None:
   f = open(position_file2)
   reader = csv.reader(f)
   l = [row for row in reader]
   position_list2 = [[int(v) for v in row] for row in l]
   #print("position_list2:"+str(position_list2)) 

if position_file3 is not None:
   f = open(position_file3)
   reader = csv.reader(f)
   l = [row for row in reader]
   position_list3 = [[int(v) for v in row] for row in l]
   #print("position_list3:"+str(position_list3)) 
...
#site_pos = [(1000,500), (1000, 1500)]
#site_pos = [(1000,500), (1000, 2500)]


#ue_pos = [(1000,490), (1000, 1510)]
ue_pos = [(1000,490), (1000, 2510)]
#if d<= 0.01:  d = 0.01
...
for i_env, i in enumerate(site_pos):
    ...
    for id_mbs in range(num_mbs_per_site):
        ...
        for _ in range(UE_per_mbs):
            ...
            if random.random() < prob_ped:
                sim.add_ue("Ped", ue_id, cell_color, ueans[0], ueans[1], mbs) #no meaning ped,veh
                #sim.add_ue("Ped", ue_id, cell_color, ue_pos[i_env][0], ue_pos[i_env][1], mbs) #no meaning ped,veh
            else:
                sim.add_ue("Veh", ue_id, cell_color, ueans[0], ueans[1], mbs)
                #sim.add_ue("Veh", ue_id, cell_color, ue_pos[i_env][0], ue_pos[i_env][1], mbs)
            ue_id = ue_id + 1
...
for i in range(x_lim_l, x_lim_r+x_grid, x_grid):
    for j in range(y_lim_l, y_lim_r+y_grid, y_grid):
        ...
        for _ in range(UE_per_sbs):
            ...
            ueans = find_xy_c_mobility(x_grid, y_grid, x_lim_l, y_lim_l, x_lim_r, y_lim_r)
            if random.random() < prob_ped:
                sim.add_ue("Ped", ue_id, cell_color, ueans[0], ueans[1], sbs)#no meaning ped,veh
            else:
...
sim.execute_sim_darwinian(position_list1)
...
#for i in range(100):
    ...
    if (i<5000):
        ...
        print("task2:"+str(i))
        sim.execute_sim_darwinian(position_list3)
