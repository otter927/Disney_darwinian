...
class UE_mobility():
    ...
    def __init__(self, x, y, speed, kind):
        ...
        self.direction = 3 #3 #random.randint(0,3)
    ...
    def get_position_test(self, ue_i):
        if ue_i == 0:
            self.pos_i = (self.pos_i + 1) % len(self.position_list)
        ans = self.position_list[(self.pos_i) % len(self.position_list)]
        return(ans)
...
ped_speed = 1 * 60 #(4km/h) -> (1m/s) -> (60m/min) simulation is 1min interval
veh_speed = 15 * 60 #(60km/h) -> (15m/s) -> (900m/min)
uav_speed = 50 * 60
...
UE_ped_list = []
UE_veh_list = []
for i in range(x_lim_l, x_lim_l+x_grid, x_grid):
#for i in range(x_lim_l, x_lim_r+x_grid, x_grid):
    #for j in range(y_lim_l, y_lim_l+y_grid, y_grid):
...
for ue in UE_ped_list:
    ...
    for i in range(repeat_scale):
        ...
        #    moving = True
        #if (moving == True) and (ue.bool_return() == True): # make roop
        #    print(ue.get_i())
        #    break
...
#for i in range(600):
    ...
    for j in range(ped_times): #14
        for ue in UE_ped_list_select:
            #print("ped:"+str(j * 10))
#            xy = ue.get_position(j * 100)
            xy = ue.get_position_test(j * 10)
...
f = open("data/position"+args[1]+"_"+args[2]+"_3_"+args[3]+".csv", 'w')
writer = csv.writer(f)
