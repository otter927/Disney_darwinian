...
#define RECOMBINATION 0.1//0. //(0.1)    // probability of recombining two output sequences; if not recombination, migration
...
void Darwinian_deme::updateOutput2(float *v, float *w, int d, int n, int i) { // update of neuron `i` in network `n` in deme `d` with threshold neuron model with input `v`; output is written into `w`
   int j;
   float h = 0.0;
   for(j = 0; j < N; j++) if(j != i) h += (deme[d].weight[n][i][j]) * v[j];
   if(h >= THRESHOLD) w[i] =  1.0;
   else               w[i] = -1.0;
}
