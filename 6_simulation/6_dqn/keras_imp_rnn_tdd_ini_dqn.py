...
#                    dict(type='dense', size=4*40, activation='leaky-relu')]
    network_spec = [dict(type='dense', size=16*90, activation='tanh'),
                    dict(type='dense', size=16*90, activation='tanh')]
    memory = 100000; batch_size = 50000; lr = 0.05; freq = 10 #lr = 5e-4;
    print("before agent")
    print(env.states())
    print(env.actions())
    agent = Agent.create(
