...
def tdd_renew_foward():
    ...
    if (len(learning_input.shape)>1):
        learning_input_tmp = learning_input.reshape(learning_input.shape[0],180,80);
        learning_input_tmp = learning_input_tmp[-1]
    else:
        learning_input_tmp = learning_input.reshape(180,80);
    print(learning_input_tmp.shape)

    foward_input = np.loadtxt("foward_input.txt");
    print("foward_input.shape")
    print(foward_input.shape)
    foward_input_tmp = foward_input.reshape(180,80);
    print(foward_input_tmp.shape)

    print("foward_input_tmp.shape")
    foward_input_tmp = np.delete(foward_input_tmp, 0, 0)
    print(foward_input_tmp.shape)
    foward_input_tmp = np.append(foward_input_tmp, learning_input_tmp[-1])
    print(foward_input_tmp.shape)
