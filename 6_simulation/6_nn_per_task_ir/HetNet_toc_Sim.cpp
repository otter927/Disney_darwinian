...
double Simulator::capacity_calculation3(vector<vector<vector<double>>> & sinr_map_all){
        ...
        for (Link *i : Link_list) {        
	      ...
	      //printf("first BS.id: %d\n", (*(*i).get_bs()).id);
		      ...
		      sinr_db = sinr_map_all[sinr_index][x_pos][y_pos] -rand_normal(0.0, 1.0);
                        ...
                        #ifdef DEBUG
                        printf("capacity, sinr, direction, BS.id: %f, %f, %d, %d\n", capacity, sinr_db, (*i).get_direction(), (*(*i).get_bs()).id);
                        #endif // DEBUG
                        (*i).set_capacity(capacity);
                        (*i).set_sinr(sinr_db);
                    } else {
...
double Simulator::execute_sim_get_UE_throughput(vector<double> &position_list, FILE *fi){
        ...
        printf("flagistrue:%d\n", ul_dl_rate_vector.size());
        for (int i=ul_dl_rate_vector.size()-80; i<ul_dl_rate_vector.size()-1; i++){
...
double Simulator::execute_sim_supervised_evaluation(vector<double> &position_list, vector<vector<double>> &foward_list, vector<vector<vector<double>>> & sinr_map_all, FILE *fi, vector<FILE *>fo_list, int file_num_tmp){
...
double Simulator::execute_sim_dqn_evaluation(vector<double> &position_list, vector<vector<double>> &foward_list, vector<vector<vector<double>>> & sinr_map_all, FILE *fi, FILE *fr, vector<FILE *>fo_list, int file_num_tmp){

    double reward;
    reward = execute_sim_supervised_evaluation(position_list, foward_list, sinr_map_all, fi, fo_list, file_num_tmp);
