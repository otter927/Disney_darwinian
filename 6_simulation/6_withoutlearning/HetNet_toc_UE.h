...
class UE
{
    ...
    vector<double> sentdemanddata_ul_list;
    vector<double> sentdemanddata_dl_list;
    
    
    
    int max_id = 0;
    double data_size;// = 50.0;//0.5; //#8 #Mbyte 0.5MB * pow(10, 6) 
    ...
    UE(Simulator *sim_x, string UEtype, int id_x, int cell_color_x, double x_pos, double y_pos, double z_pos, BS *bs_x, double ul_rate_x, double data_size_x);
    ...
    void append_demandsentdata(Data *d, double capacity, int direction);
    double get_ue_sentdata_total();
    ...
    int get_id();
    bool not_at_gate();
    void gen_data(BS *bs, double data_size_x, int direction);
