...
def tdd_renew_foward():
    ...
    if (len(learning_input.shape)>1):
        learning_input_tmp = learning_input.reshape(learning_input.shape[0],40,16);
        learning_input_tmp = learning_input_tmp[-1]
    else:
        learning_input_tmp = learning_input.reshape(40,16);
    print(learning_input_tmp.shape)

    foward_input = np.loadtxt("foward_input.txt");
    print("foward_input.shape")
    print(foward_input.shape)
    foward_input_tmp = foward_input.reshape(40,16);
