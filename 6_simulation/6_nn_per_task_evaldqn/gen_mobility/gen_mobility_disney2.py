...
class UE_mobility():
    ...
    def if_c_flag(self):
        if (self.source_id == (self.dist_id + 1)%9) or (self.source_id == (self.dist_id - 1)%9) or (self.source_id == self.dist_id):
...
tranmatrix_file = np.loadtxt('data/tranmatrix_20220914.txt')
len_tmp = int(tranmatrix_file.shape[0] / 9)
print(len_tmp)
transit_array_tmp = tranmatrix_file.reshape((len_tmp,9,9))
...
init_UE_num = uenum_array[0];
tmp = (max_sum - np.sum(uenum_array[0]))
#print(max_sum)
#print(np.sum(uenum_array[0]))
#print(np.sum(init_UE_num))
#print(tmp)
init_UE_num = np.append(init_UE_num, tmp)
print(init_UE_num)

num_UE = 0
for i in range(9):
    for j in range(9):
        for k in range(int(transit_array[0][i][j]*init_UE_num[i])):
