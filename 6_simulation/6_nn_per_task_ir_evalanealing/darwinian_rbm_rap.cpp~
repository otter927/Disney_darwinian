...
#define RECOMBINATION 0.0//0.0//0.1 //(0.1)    // probability of recombining two output sequences; if not recombination, migration
#define MIGRATION     0.004 //(0.004)   // probability of migration (only if DN > 1)
#define MUTATION      1.0/(float)N//(1.0/(float)N)//(1.0/(float)N) // per-bit mutation rate of selected re-entered output of previous generation
...
Darwinian_deme::Darwinian_deme(int n_NN) {
    NN = n_NN;
    N = 16;
    ENVNUM = 2;
    P = 10;
    ...
    DN = 1;
    PN = N;
    learnCounter.resize(DN, vector<int>(NN));
    globalOptimumS.resize(N);
    globalOptimumAll.resize(ENVNUM, vector<float>(N));
    blockOptimaSequence.resize(B, vector<vector<float>>(T, vector<float>(P)));
    blockOptimaFitness.resize(B, vector<float>(T)); 
    matrixNeighbors.resize(DN, vector<int>(8));
    for (int i=0; i<DN; i++){
        deme.push_back(DEME(NN));
    }
}
...
void Darwinian_deme::train_rbm(vector<float> &v, int d, int n){
    int i, j, k;
    vector<float> v_0(n_v), p_h_0(n_h), p_h(n_h);//, v_1[n_v];
...
void Darwinian_deme::encode_rbm(vector<float> &v, int d, int n){

    int j, k;
    vector<float> p_h(n_h), h(n_h), p_v(n_v);
...
void Darwinian_deme::encode_rbm2(vector<float> &v, vector<float> &w, int d, int n){
...
void Darwinian_deme::encode_decode_rbm(vector<float> &v, vector<float> &p_h, int d, int n){

    int i, j, k;
    vector<float> h(n_h), p_v(n_v);
...
void Darwinian_deme::update_rbm(vector<float> &v_0, vector<float> &v, vector<float> &p_h_0, vector<float> &p_h, int d, int n){
...
float Darwinian_deme::pearsonCorrelation(vector<float> &u, vector<float> &v) { // Pearson product-moment correlation coefficient of vectors `u` and `v`
  int i;
...
float Darwinian_deme::pearsonFitness(vector<float> &v) {
	return(pearsonCorrelation(v, globalOptimumS));
}

int Darwinian_deme::hammingDistanceN(vector<float> &v, vector<float> &u, int n) { // standard HD up to length `n`
  int i, d = 0;
  for(i = 0; i < n; i++) if(v[i] != u[i]) d++;
  return(d);
}

int Darwinian_deme::hammingDistance(vector<float> &v, vector<float> &u) { // standard HD of length N
  int i, d = 0;
  for(i = 0; i < N; i++) if(v[i] != u[i]) d++;
  return(d);
}

float Darwinian_deme::relativeHammingDistance(vector<float> &v, vector<float> &u) { // HD/N
  return((float)hammingDistance(v, u)/(float)N);
}

float Darwinian_deme::hammingFitness(vector<float> &v) { // 1 - (HD/N)
  return(1.0 - relativeHammingDistance(v, globalOptimumS));
}

int Darwinian_deme::randomMinPosition(vector<float> &v, int n) { // selects the position of the smallest value in vector `v` up to length `n`; if there are multiple instances, selects one randomly.
	float found;
...
int Darwinian_deme::randomMaxPosition(vector<float> &v, int n) { // selects the position of the largest value in vector `v` up to length `n`; if there are multiple instances, selects one randomly.
	float found;
	int i, count = 0, r, pos[n];
	for(i = 0; i < n; i++) pos[i] = -1;
	for(i = 0; i < n; i++) {
...
int Darwinian_deme::firstMaxPosition(vector<float> &v, int n) { // selects the position of the largest value in vector `v` up to length `n`; if there are multiple instances, selects last (to the right).
	float max = -999.;
	int i, pos = -1;
...
int Darwinian_deme::samePatternQ(vector<float> &u, vector<float> &v) { // boolean test of pattern identity
	int i = 0, q = 1;
        //printf("i, N, q: %d, %d, %d\n", i, N, q);
        while((i < N) && q) {
               //printf("u[i], v[i]: %d, %f, %f\n", i, u[i], v[i]);
		if(u[i] != v[i]) q = 0;
		i++;
               //printf("i, N, q: %d, %d, %d\n", i, N, q);
	}
...
void Darwinian_deme::copyPattern(vector<float> &to, vector<float> &from_t) {
	int i;
	for(i = 0; i < N; i++) to[i] = from_t[i];
}

void Darwinian_deme::copyPatternN(vector<float> &to, vector<float> &from_t, int n) {
	int i;
	for(i = 0; i < n; i++) to[i] = from_t[i];
}

void Darwinian_deme::copyPatternNN(vector<float> &to, vector<float> &from_t) {
	int i;
	for(i = 0; i < NN; i++) {
	    printf("a:%f\n", from_t[i]);
	    printf("b:%f\n", to[i]);
...
void Darwinian_deme::invertPattern(vector<float> &v) { // flip each bit in vector `v`
	int i;
	for(i = 0; i < N; i++) v[i] = -1.0 * v[i];	
}

void Darwinian_deme::printPattern(vector<float> &v) {
	int i;
	for(i = 0; i < N; i++) {
		if      (v[i] == -1.) printf("-");
...
//const char *Darwinian_deme::vectorToString(vector<float> &v, int n) { // cannot be called multiple times in e.g. printf!
vector<char> Darwinian_deme::vectorToString(vector<float> &v, int n) { // cannot be called multiple times in e.g. printf!
	int i;
...
void Darwinian_deme::mutatePattern(vector<float> &v, float mut) { // mutate pattern per-digit mutation rate `mut`
	int i;
	if(mut > 0.0) for(i = 0; i < N; i++) if(randd() < mut) v[i] = -1. * v[i];
}

void Darwinian_deme::exactlyMutatePattern(vector<float> &v, float mut) { // mutates exactly `mut*N` bits (maximally N)
...
void Darwinian_deme::mutateSinglebitPattern(vector<float> &v, float mut) {
	int i;
	i = randl(N/2);
	v[N/2 + i] = -1. * v[N/2 + i];
}

void Darwinian_deme::randomPattern(vector<float> &v, float mut) { // generates a random pattern with `mut` probability that a bit is +1
	int i;
	for(i = 0; i < N; i++) v[i] = (randd() < mut) ? 1.0 : -1.0;
}

void Darwinian_deme::alternatingPattern(vector<float> &v, int l, int start) { // alternating +1.0 and -1.0 with partition length `l`, starting with `start` (1 -> +1.0, else -> -1.0) 
	int i, b = 1;
	for(i = 0; i < N; i++) {
		v[i] = (b)?(+1.0):(-1.0);
		if(((i+1) % l) == 0) b = 1 - b;
...
void Darwinian_deme::continuousPattern(vector<float> &v, int from_t, int to) { // [---..+++..---] where + subpattern runs from `from` to `to-1`
	int i, t;
...
void Darwinian_deme::correlatePattern(vector<float> &v, float corr) { // generate a partially uncorrelated version of vector `v` (saving back to `v`)
	int i;
  vector<float> x(N);
	if(corr < 1.0) {
		if(corr == -1.0) {
...
void Darwinian_deme::recombinePattern(vector<float> & u, vector<float> & v) { // two-point recombination
...
void Darwinian_deme::storePattern(vector<float> &v, int d, int n) { // tests if `v` is stored in network `n` at deme `d` or not, and pushes/stores it to most recent position (on the right)
  // If there is an identical in `stored`, remove it, shift those right of it one step to the left and store the new one at the last position
	int j = PN-1;
	//printf("j:%d\n", j);
	//printf("deme[d].stored[n][j].size():%d\n", deme[d].stored[n][j].size());
	while(!samePatternQ(v, deme[d].stored[n][j]) && j >= 0){ 
	    j--;
	    //printf("j*:%d\n", j);
	    if (j == 0){
	        break;
...
void Darwinian_deme::trainNetwork(vector<float> &v, int d, int n) { // training network `n` in deme `d` with vector `v`, updating `weight`
//  int i, j;
...
void Darwinian_deme::updateOutput(vector<float> &v, int d, int n, int i) { // update of neuron `i` in network `n` in deme `d` with threshold neuron model
   int j;
   float h = 0.0;
...
void Darwinian_deme::updateNetwork(vector<float> &v, int d, int n) { // updating network 'n' in deme `d` with input `v`; output is written into `v`
   //int i, u;
...
void Darwinian_deme::updateNetwork2(vector<float> &v, vector<float> &w, int d, int n) { // updating network 'n' in deme `d` with input `v`; output is written into `w`
   //int i, u;
...
int Darwinian_deme::closestStored(vector<float> &v, int d, int n) { // returns the index of the stored pattern that is closest to `v` in deme `d`, network `n`
	int j;
	vector<float> hd(PN);
...
void Darwinian_deme::blockGlobalOptimumSequence(vector<float> &v) { // calculates global optimum using `blockOptimaSequence` and `blockOptimaFitness`
	int i, j, bestP;
	float bestW;
	for(i = 0; i < B; i++) {
		bestW = 0.0; // best fitness
...
float Darwinian_deme::blockFitness(vector<float> &v) { // building block fitness; rescales range (min, max) to (min/max, 1.0)
	int i, j, d;
	float c, sum = 0.0;
	vector<float> block(P);
	for(i = 0; i < B; i++) {
...
vector<DEME> Darwinian_deme::evolution_gen_update(int t) {
				...
				if(TYPE == 4) {for(i = 0; i < NN; i++) {updateNetwork(deme[d].output[i], d, i);        
				                //printf("c\n");
                                        ...
                                        for(j=0; j<i; j++){
                                               ...
                                               hd += relativeHammingDistance(deme[d].output[i], deme[d].output[j]);
                                        }
...
vector<DEME> Darwinian_deme::evolution_gen_update_supervised(int t, vector<vector<double>> &foward_list) {
    ...
    #ifdef DEBUG
    printf("Darwinian_rbm_rap evolution_gen_update_supervised1\n");
    #endif
    ...
    for(d = 0; d < DN; d++) {
        ...
        #ifdef DEBUG
        printf("Darwinian_rbm_rap evolution_gen_update_supervised2\n");
        #endif
        ...
        for(i = 0; i < NN; i++) {
                ...
                #ifdef DEBUG
                printf("Darwinian_rbm_rap evolution_gen_update_supervised31\n");
                #endif
                
                printf("deme.size():%d\n", deme.size());
                printf("deme[%d].output[%d].size():%d\n", d, i, deme[d].output[i].size());
                printf("foward_list[%d].size():%d\n", i, foward_list[i].size());
                printf("N:%d\n", N);
                //#ifdef DEBUG
                //printf("output_vector:");
                //cout << foward_list[i][j] << endl;
                //#endif // DEBUG

                for (j=0; j<N; j++){
                    deme[d].output[i][j] = (float)foward_list[i][j];
                    #ifdef DEBUG
                    printf("output_vector: %d, %f, %f\n", j, foward_list[i][j], deme[d].output[i][j]);
                    #endif // DEBUG
                }
				                
                for(j=0; j<i; j++){
                    printf("deme[%d].output[%d].size():%d,deme[%d].output[%d].size():%d", d, i, deme[d].output[i].size(), d, i, deme[d].output[j].size());
                    hd += relativeHammingDistance(deme[d].output[i], deme[d].output[j]);
                }
...
vector<float> Darwinian_deme::evolution_gen_calcbestfitness(int t) {

    	        int i, j, k, d, e = 0; 
    	        int bestD = -99, bestP = -99;
	        float avgW = 0, bestW = -99000., worstW = 10000.;
		...
		for(d = 0; d < DN; d++) {
			...
			//if(deme[d].minW < 1000) {
			//        if (deme[d].minW < wworstW){
			//                wworstW = deme[d].minW;
			//                copyPattern(deme[d].wworstoutput, deme[d].output[deme[d].minP]);
			//        }
			//}			
			avgW += deme[d].avgW;
		}
		...
		if(!(t % (RESOLUTION))) {
			...
			printf("darwinian: %d\t%f\t%f\t%f\t%d\t", t, bestW, avgW/(float)DN, worstW, bestP);
...
bool Darwinian_deme::evolution_gen_replacemutation(int t) {
			...
			for(d = 0; d < DN; d++) {
				/* Replication & learn */
        #ifdef DEBUG
        printf("!!Darwinian_rbm_rap evolution_gen_replacemutation\n");
        printf("deme[d].w1: %f, deme[d].min: %f\n", deme[d].w1, deme[d].minW);
        #endif // DEBUG
                               //exit(0);
...
void Darwinian_deme::evolution_gen_learnbest(int t) {

        	        int i, j, k, d, e = 0; 

			for(d = 0; d < DN; d++) {
				/* Replication & learn */
                		//if(REPLACE == 0) {copyPattern(deme[d].output[deme[d].minP], deme[d].output[deme[d].maxP]);} else // replace the worst patterns in the output (choosing one randomly, if there are multiple)
				//if(REPLACE == 1) {for(i = 0; i < NN; i++); copyPattern(deme[d].output[i], deme[d].output[deme[d].maxP]);} else // replace ALL of the output population with the new output
				//if(REPLACE == 2) {copyPattern(deme[d].output[randl(NN)], deme[d].output[deme[d].maxP]);} // replace ONE pattern randomly
				...
				if((TYPE >= 4) && (t < MAXTRAINT)) { // Learning
					...
					for(k = 0; k < ((TRAINNUM>NN)?NN:TRAINNUM); k++) { // the best pattern is assured to be trained to TRAINNUM *different* networks (faster convergence)
						...
						//printf("darwiniankoko:");

        					do i = randl(NN); while(pos[i]);
                        			pos[i] = 1;
						copyPattern(toLearn, deme[d].output[deme[d].maxP]);
						mutatePattern(toLearn, TRAINNOISE);
						trainNetwork(toLearn, d, i);
...
void Darwinian_deme::evolution_gen_learnworst(int t) {

        	        int i, j, k, d, e = 0; 

			for(d = 0; d < DN; d++) {
				/* Replication & learn */
                		//if(REPLACE == 0) {copyPattern(deme[d].output[deme[d].minP], deme[d].output[deme[d].minP]);} else // replace the worst patterns in the output (choosing one randomly, if there are multiple)
				//if(REPLACE == 1) {for(i = 0; i < NN; i++); copyPattern(deme[d].output[i], deme[d].output[deme[d].minP]);} else // replace ALL of the output population with the new output
				//if(REPLACE == 2) {copyPattern(deme[d].output[randl(NN)], deme[d].output[deme[d].minP]);} // replace ONE pattern randomly
				...
				if((TYPE >= 4) && (t < MAXTRAINT)) { // Learning
					...
					for(k = 0; k < ((TRAINNUM>NN)?NN:TRAINNUM); k++) { // the best pattern is assured to be trained to TRAINNUM *different* networks (faster convergence)
						...
						//printf("darwiniankoko:");
        					...
        					do i = randl(NN); while(pos[i]);
                        			pos[i] = 1;
						copyPattern(toLearn, deme[d].output[deme[d].minP]);
						mutatePattern(toLearn, TRAINNOISE);
						trainNetwork(toLearn, d, i);
...
vector<DEME> Darwinian_deme::evolution_gen_selection(int t) {

        	        int i, j, k, d, e = 0; 
        	        
        #ifdef DEBUG
        printf("!!Darwinian_rbm_rap Evolution_gen_selection1\n");
        #endif // DEBUG
        ...
        #ifdef DEBUG
        printf("!!Darwinian_rbm_rap Evolution_gen_selection2\n");
        #endif // DEBUG
        ...
        #ifdef DEBUG
        printf("!!Darwinian_rbm_rap Evolution_gen_selection3\n");
        #endif // DEBUG
        ...
        #ifdef DEBUG
        printf("!!Darwinian_rbm_rap Evolution_gen_selection4\n");
        #endif // DEBUG
        ...
        #ifdef DEBUG
        printf("!!Darwinian_rbm_rap Evolution_gen_selection5\n");
        #endif // DEBUG
        ...
        #ifdef DEBUG
        printf("!!Darwinian_rbm_rap Evolution_gen_selection6\n");
        #endif // DEBUG
					recombinePattern(deme[d].v1, deme[d].v2);
				} else {
        #ifdef DEBUG
        printf("!!Darwinian_rbm_rap Evolution_gen_selection7\n");
        #endif // DEBUG
					deme[d].recombination = 0;
					/* Mutate */
					copyPattern(deme[d].v1, deme[d].output[p1]);
					mutatePattern(deme[d].v1, (MUTATION));
					copyPattern(deme[d].v2, deme[d].output[p1]); //added
				}
                       }
...
void Darwinian_deme::return_fitness(vector<float> &w, int d) {
        //printf("H\n");
        //printf("%f\t%f\t%d\n", w[0], w[1], d);
...
void Darwinian_deme::return_fitness_mutate(vector<float> &w, int d) {
        //printf("w1%f\t%f\n", deme[d].w1, w[0]);
        //printf("w2%f\t%f\n", deme[d].w1, w[1]);
        deme[d].w1 = w[0];
        deme[d].w2 = w[1];
	...
	while (t <= MAXT) {
		...
		if(t > 0) { // skip this in the first turn
		    ...
		    evolution_gen_replacemutation(t);
  		    ...
  		    //printf("darwiniankokob:");
		    evolution_gen_learnbest(t);
		    
		    evolution_gen_learnworst(t);
