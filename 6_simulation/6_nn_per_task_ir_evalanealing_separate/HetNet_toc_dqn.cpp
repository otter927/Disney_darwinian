...
void getData_sinrmap(vector<vector<vector<double>>> & sinr_map_all){
    ...
    #ifdef DEBUG
    printf("HetNet_toc_supervised getData_sinrmap!\n");
    #endif // DEBUG
...
//exit(0);
    ...
    char buff_ul_dl[30000];
    char buff_ul_dl_tmp[30000];
    vector<double> ul_dl_rate_vector_tmp;
    vector<float> ul_dl_rate_vector;

    int buff_flag = 0;
    while(fgets(buff_ul_dl, 30000, (FILE*)firead) != NULL){
    ...
    FILE *fi = fopen(folder_learning_input, "a");
    char *num;
    ...
    for (int i=0; i<atoi(argv[3]); i++){
        ...
        FILE *fo = fopen(folder_learning_output.c_str(), "a");
        fo_list.push_back(fo);
    }
 
 
    char folder_learning_reward[] = "./learning_reward.txt";
    FILE *fr = fopen(folder_learning_reward, "a");

    char folder_learning_reward_output[] = "./learning_reward_output_tmp.txt";
    FILE *fro = fopen(folder_learning_reward_output, "a");
...
//exit(0);
    ...
    srand(0);
    double data_size_x = atof(argv[4]);
	...
	double rand_x = rand_exp();
        UE *ue = new UE(sim, "Ped", ue_id, cell_color_tmp, x_ue, y_ue, 0.0, t_sbs, rand_x, data_size_x);
        #ifdef DEBUG
        here = clock();
...
//    for (int i=0; i<2; i++){
    ...
    //getData(buff, position_list);
    (*sim).execute_sim_dqn_evaluation(position_list, foward_list, sinr_map_all, fi, fr, fro, fo_list, atoi(argv[3]), atoi(argv[2]), atoi(argv[5]));
...
//exit(0);
        ...
        getData(buff, position_list);
        (*sim).execute_sim_supervised_evaluation(position_list, foward_list, sinr_map_all, fi, fo_list, atoi(argv[3]));
