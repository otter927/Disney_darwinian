...
double Simulator::execute_sim_supervised(vector<double> position_list){
...
////////////////////////////////
        ...
        //for o_env, output in enumerate(deme_list):
            ...
            // Load an image
            //vector<float> v1{1,  9,  1,  9,  1,  9,  1,  9,  1,  9,  1,  9,  1,  9,  1,  9, 1,   1,  9,  1,  9,  1,  9,  1,  9,  1,  9,  1,  9,  1,  9, 1,  9,  1,  9,  1,  9,  1,  9,  1,  9,  1,  9,  1,  9,  1,  9, 1,  9,  1,  9,  1,  9,  1,  9,  1,  9,  1,  9,  1,  9,  1,  9};
            vector<int64_t> vector_size{1, 4, 16};
            ...
            auto input = cppflow::tensor(ul_dl_rate_vector, vector_size);
            ...
            // Run
            auto output_tensor = model({{"serving_default_simple_rnn_input:0", input}},{"StatefulPartitionedCall:0"});
    
            output_vector = output_tensor[0].get_data<float>();
    
            #ifdef DEBUG
            printf("output_tensor:");
            cout << output_tensor[0] << endl;
            #endif // DEBUG
        ...
        //for o_env, output in enumerate([deme_list[0], deme_list[1]]):
            ...
            for (BS *bs : BS_list){
                if (n==0){
                    (*bs).sim_set_config_supervised_learning(output_vector_var1);
                }else{
                    (*bs).sim_set_config_supervised_learning(output_vector_var2);
                }
            }
        ...
        for (int j=0; j< 10; j++){
            ...
            for (BS *bs : BS_list){
                (*bs).sim_set_config_supervised_learning(best_output_vector);
                #ifdef DEBUG
                here = clock();
                printf("!!HetNet_toc_Sim execute_sim_darwinian61, %.2f\n", (double)(here - start)/1000);
                start = here;
