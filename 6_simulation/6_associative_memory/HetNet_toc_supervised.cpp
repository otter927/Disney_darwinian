...
double rand_exp(){
  ...
  if (ans > 1.0){
    //printf("rand_exp:%f", 1.0);
    return(1.0);
    //return(0.0);
  }else{
    //printf("rand_exp:%f", ans);
    return(ans);
    //return(1.0-ans);
  }
}
...
//exit(0);
    ...
    printf("data_size_x2:%f", data_size_x);
...
//    for (int i=0; i<2; i++){
    ...
    //getData(buff, position_list);
    (*sim).execute_sim_supervised_evaluation(position_list, foward_list, sinr_map_all, fi, fo_list, atoi(argv[3]), atoi(argv[2]));
