            ...
            if (d->direction == 0){
                ...
                if (up_link != NULL){
                    ...
                    if ((*d->bs).get_state() == 0){
                        ...
                        #ifdef DEBUG
                        printf("!HetNet_toc_UE send capacity: %e, %e, %d\n", capacity, d->remaining_data_size, (*up_link).id);
                        #endif // DEBUG
            ...
            if (d->direction == 1){
                ...
                if (up_link != NULL){
                    ...
                    if ((*d->bs).get_state() == 1){
                        capacity = (*up_link).get_capacity();
                        #ifdef DEBUG
                        printf("!HetNet_toc_UE recieve capacity: %e, %e\n", capacity, d->remaining_data_size);
                        #endif // DEBUG
                        if (capacity > 0.000001){
	                            d->send_begin = 1;
                            //#print("uplink capacity, bs:" + str(capacity)+","+str(bs.get_name()))
