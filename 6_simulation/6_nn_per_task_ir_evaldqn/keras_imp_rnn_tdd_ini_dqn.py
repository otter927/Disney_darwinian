...
def tdd_ini(args1):
    ...
    X = np.zeros(80*180)
    print(X)
    observation = np.zeros(80*180)


    #y = np.random.randint(0,2,(4,16)) * 2 - 1
    #y = np.random.randint(0,2,(4,16)) * 2 - 1
    y = np.random.randint(0,2,80) * 2 - 1
    print(y)
...
#                    dict(type='dense', size=4*40, activation='leaky-relu')]
    network_spec = [dict(type='dense', size=80*180, activation='tanh'),
                    dict(type='dense', size=80*180, activation='tanh')]
    ...
    agent = Agent.create(
        agent='double_dqn',
        states={'type':'float', 'shape':(80*180,)},#env.states(),
        actions={'type':'int', 'num_values':2, 'shape':(80,)},#{'type':'int', 'num_values':16},#env.actions(),
        memory=memory,
...
#                    dict(type='dense', size=4*40, activation='leaky-relu')]
    ...
    agent = Agent.create(
        agent='double_dqn',
        states={'type':'float', 'shape':(80*180,)},#env.states(),
        actions={'type':'int', 'num_values':num, 'shape':(1,)},#{'type':'int', 'num_values':16},#env.actions(),
        memory=memory,
        batch_size=batch_size,
        exploration=0.05,#0.05,0.9
        # learning_rate=dict(type='exponential', unit='episodes', num_steps=50, initial_value=lr, decay_rate=0.8),
