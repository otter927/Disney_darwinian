...
def tdd_foward_learn_dqn(agent):
    ...
    if (len(text.shape)>1):
        newtext = text.reshape(text.shape[0],180,80);
        newtext = newtext[-1];
    else:
        newtext = text.reshape(180,80);        
    #print(newtext.shape)
    ...
    if (len(learning_reward_output.shape)>1):
        learning_output = learning_reward_output[-1, actions[0]*80:actions[0]*80+80]
    else:
        learning_output = learning_reward_output[actions[0]*80:actions[0]*80+80]
