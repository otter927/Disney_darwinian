...
double UE::get_TX_power(void){
        ...
        #ifdef DEBUG
        cout << "!HetNet_toc_UE get_TX_power\n";
        printf("bs.get_type: %s\n", (*inside_BS).get_type().c_str());
        #endif // DEBUG
        
        if ((*inside_BS).get_type() ==  "MBS"){
            tmp_power = 10 * log10(M) + power_control_MBS_UE + power_control_alpha * (*sim).path_loss(inside_BS, this, 0);
            #ifdef DEBUG
            cout << "!HetNet_toc_UE get_TX_power2\n";
            printf("power_control_MBS_UE, power_control_alpha, path_loss, tmp_power: %f, %f, %f, $f\n", power_control_MBS_UE, power_control_alpha, (*sim).path_loss(inside_BS, this, 0), tmp_power);
            #endif // DEBUG
        }else{
            tmp_power = 10 * log10(M) + power_control_SBS_UE + power_control_alpha * (*sim).path_loss(inside_BS, this, 0);
            #ifdef DEBUG
            cout << "!HetNet_toc_UE get_TX_power3\n";
            printf("power_control_SBS_UE, power_control_alpha, path_loss, tmp_power: %f, %f, %f, %f\n", power_control_SBS_UE, power_control_alpha, (*sim).path_loss(inside_BS, this, 0), tmp_power);
            #endif // DEBUG
        }
...
BS *UE::find_BS(BS *old_bs){
        ...
        for (BS *bs : (*sim).BS_list){
            if ((*bs).get_type() == "SBS"){
                ...
                #ifdef DEBUG
                printf("HetNet_toc_UE find_BS bs_id, distance: %d, %f, %f, %f\n", (*bs).id, distance, x_position, y_position);
                #endif // DEBUG
