...
double Simulator::signal(BS *lbs, UE *lue, int direction){
        ...
        #ifdef DEBUG
        cout << "!HetNet_toc_Sim signal\n";
        printf("TX, ue_antenna, bs_antenna, ue_noise, bs_noise, shadowing, path_loss: %f, %f, %f, %f, %f, %f, %f\n", ans1, ans2, ans6, ans5, ans7, ans4, ans3);        
        #endif // DEBUG
...
//#        sinr_bottom = interf + d2w(reciever.noise(r1))
        ...
        for (Link *i : Link_list) {        
         //for (i = 1; i <= sizeof(self.Link_list) / sizeof(self.Link_list[0]); i++){
            ...
            printf("Link, BStype, %d, %s\n", (*i).id, (*(*i).get_bs()).get_type().c_str());
            #endif // DEBUG
            ...
            if ((*i).is_active() == true){
                ...
                printf("SINR db: %s, %s, %f, %e, %d, %d\n", (*(*i).get_bs()).get_name().c_str(), (*(*i).get_ue()).get_name().c_str(), w2d(sinr), sinr, (*(*i).get_ue()).x_position, (*(*i).get_ue()).y_position);
                #ifdef DEBUG
                printf("bandwidth: %f\n", bandwidth);
                #endif // DEBUG
...
double Simulator::execute_sinrmap(){
        ...
        for (int i=4000; i<4700; i=i+10){
            for (int j=-41100; j<-40400; j=j+10){
                ...
                (*ue_tmp).gen_link(new_bs ,ue_tmp, 0);
